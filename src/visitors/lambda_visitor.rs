            ...
            if let_expr.as_str().contains("filter(") {
                            ...
                            while let Some(inner_arg_list_pair) = inner_arg_list_pairs.next() {
                                ...
                                match inner_arg_list_pair.as_rule() {
                                    Rule::comma_delimited_exprs => {
                                        let mut inner_comma_delimited_exprs_pairs =
                                            inner_arg_list_pair.clone().into_inner();
                                        while let Some(inner_comma_delimited_exprs_pair) =
                                            inner_comma_delimited_exprs_pairs.next()
                                        {
                                            ...
                                            match inner_comma_delimited_exprs_pair.as_rule() {
                                                Rule::expr => {
                                                    let mut inner_comma_delimited_exprs_expr_pairs =
                                                        inner_comma_delimited_exprs_pair
                                                            .clone()
                                                            .into_inner();
                                                    while let Some(
                                                        inner_comma_delimited_exprs_expr_pair,
                                                    ) = inner_comma_delimited_exprs_expr_pairs
                                                        .next()
                                                    {
                                                        ...
                                                        match inner_comma_delimited_exprs_expr_pair
                                                            .as_rule()
                                                        {
                                                            Rule::expr_inner => {
                ...
                if is_seq_type {
                        ...
                        (closure_expr, filter_var_name)
                    {
                        ...
                        if let (Some(ref closure_param_list), Some(ref filter_expr)) =
                            (closure_param_list, filter_expr)
                        {
                            ...
                            let mut closure_param_names: Vec<String> = Vec::new(); // Holds variable name and type
                                                                                   ////
                            let mut closure_param_list_params =
                                closure_param_list.clone().into_inner();
                                    ...
                                    Rule::param => {
                                        ...
                                        while let Some(inner_param_pair) = inner_param_pairs.next()
                                        {
                                            ...
                                            match inner_param_pair.as_rule() {
                                                Rule::pat_no_top_alt => {
                                                    filter_var_name =
                                                        inner_param_pair.clone().as_str();
                                                    closure_param_names
                                                        .push(filter_var_name.to_string());
                                                }
                                                _ => {}
                                            }
                                        }
                            ...
                            for variable_name in closure_param_names {
                                ...
                                expr_with_values = Some(
                                    regex
                                        .replace_all(
                                            filter_expr.as_str(),
                                            format!("{}[filter_loop_var]", filter_var_name),
                                        )
                                        .into_owned(),
                                );
                                // println!("here dddd -- {:?}", expr_with_values);
                            }
                            if let (Some(ref expr_with_values), Some(ref assign_var_name)) =
                                (expr_with_values, assign_var_name)
                            {
                                let empty_initial_assignment =
                                    format!("let mut {} = seq![];", assign_var_name);
                                let loop_var = "let mut filter_loop_var:int = 0;";
                                let seq_len_condition =
                                    format!("if(filter_loop_var < {}.len())", filter_var_name);
                                let loop_condition = format!("if({})", expr_with_values.as_str());
                                let loop_inner = format!(
                                    "{} = {}.push({}[filter_loop_var]);",
                                    assign_var_name, assign_var_name, filter_var_name
                                );
