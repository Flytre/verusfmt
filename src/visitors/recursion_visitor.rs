    ...
    fn visit_expr(
        ...
        if let (Some(function_name), Some(arguments)) = (function_name, arguments) {
            if function_name == current_parent_name {
                ...
                datum.program_mut().push_str(&format!("{}_{}({})", function_name,finite_bound,arguments));

                println!("Recursive Function called!: {} with args: {:?}",function_name, arguments);
                ...
                if let Some(original_body) = datum.fn_map.get(&function_name).cloned() {
                    ...
                    for i in 0..=finite_bound {
                        ...
                        while let Some(pos) = new_body[start..].find(&function_name) {
                            ...
                            if first_occurrence {
                                new_body.replace_range(pos..pos + function_name.len(), &new_function_name);
                                ...
                                new_body.replace_range(pos..pos + function_name.len(), &new_function_name_recursive);
                            }
